...
function DOM_i(t, n) {
	...
	switch (e.style.display = "inline-block", e.style.opacity = 0, e.style.transform = "\n        translate(" + rndint(-
			500, 500) + "%," + rndint(-500, 500) + "%)\n        scale(" + rndfloat(0, 2) + ")\n        rotate3d(" + Math
		.random() + ", " + Math.random() + ", " + Math.random() + ", " + rndint(-180, 180) + "deg)\n    ", e.innerText =
		t, n) {
		case "fast":
...
function animate() {
	p_now != -1 && ps[p_now].map(function(t) {
		t.style.transform = "\n                translate(" + rndint(-500, 500) + "%," + rndint(-500, 500) +
			"%)\n                scale(" + rndfloat(0, 2) + ")\n                rotate3d(" + Math.random() +
			", " + Math.random() + ", " + Math.random() + ", " + rndint(-180, 180) + "deg)\n            ", t
			.style.opacity = 0
	});
	do ++p_now >= ps.length && (p_now = 0); while (!ps[p_now]);
...
for (var key in GET) parseInt(key) == key ? ps[key] = GET[key] : settings[key] = GET[key];
ps.length || (ps = ["WeChat&微信 >.<", "cheng5zi1zhen5tian"], settings.size = 3);
...
var p_now = -1;
animate(), document.title = "wordsdance";
